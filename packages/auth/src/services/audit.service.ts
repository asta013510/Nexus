...
export class AuditService {
   ...
   */
  async log(event: Omit<AuditEvent, 'id'>): Promise<string> {
   ...
   */
  async getSuspiciousEvents(limit: number = 100): Promise<AuditEvent[]> {
    const result = await this.pool.query(
      `SELECT * FROM audit_logs 
   ...
   */
  async detectSuspiciousActivity(userId: string, context: {
    ipAddress: string;
    userAgent: string;
    deviceId?: string;
  }): Promise<boolean> {
