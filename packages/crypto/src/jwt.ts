...
export async function generateAccessToken(payload: {
  ...
  deviceId?: string;
  email?: string;
}, expirySeconds: number = 900): Promise<string> {
  const secret = process.env.JWT_SECRET || 'fallback-dev-secret-change-in-production';
...
export async function generateRefreshToken(payload: {
  ...
  sessionId: string;
  deviceId: string;
}, expirySeconds: number = 604800): Promise<string> {
  const secret = process.env.JWT_SECRET || 'fallback-dev-secret-change-in-production';
...
export async function verifyJWT(token: string): Promise<JWTResult> {
  try {
    ...
    return {
      valid: true,
      payload: payload as JWTPayload | RefreshTokenPayload,
    };
  } catch (error: any) {
 ...
 */
export function decodeJWT(token: string): { payload: any; header: any } | null {
  try {
